...
出典は `~/Desktop/素材バンク/職務経歴書_細川寛太.docx`（2026年9月5日現在）。

- [x] 所属 — 滋賀大学 経済学部 経済学科（2020.04 –、**在学中**）
- [x] インターン歴 — 株式会社AI'm（2026.08 –）／株式会社デルタソリューションズ（2026.01 –）
- [x] 語学 — TOEIC 875 / 英検準1級

**要確認**

- [ ] **営業インターンの数字を公開してよいか**。KPI達成率 190% / 訪問数 15→23件 / 商談化率 2%→20% は
      クライアント案件の内部指標。職務経歴書（提出先限定）と公開サイトでは意味が違うため、
      先方の了解が取れているか確認すること（要件 §5「NDA案件は非掲載」）。
      問題があれば `content/about.mdx` の該当段落から数字を落とす
- [ ] 株式会社AI'm の実績に数字が無い。職務経歴書にも
      「［ご確認：対応クライアント数・削減工数など具体的な数字があれば追記］」と残っている。
      出せる数字があれば追記すると、もう一方との対比で見劣りしない
- [ ] 塾リーダー講師の経験は載せていない（成績向上率の「滋賀県内1位」が職務経歴書側で
      「範囲の検算が必要」とされていたため）。検算して載せるかどうか
